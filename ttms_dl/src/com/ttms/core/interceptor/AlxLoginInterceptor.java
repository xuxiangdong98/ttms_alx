 ...
 *
 */
public class AlxLoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
		//获取请求的URL
		String url = request.getRequestURI();
		//判断该请求是否包含toAlxlogin.action,如果包含则放行，如果不包含则全部拦截
		if(url.indexOf("/toAlxlogin.action")>=0||url.indexOf("/alxlogin.action")>=0) {
			return true;
		}
		//对其他的请求，进行登录判断，获取Session
		HttpSession session = request.getSession();
		...
		//如果Session中没有用户数据，则说明不符合条件，给出提示信息并转发跳转到登录页面
		request.setAttribute("msg","亲，您还没有登录，请先登录！");
		request.getRequestDispatcher("/toAlxlogin.action").forward(request, response);
		return false;
	}
